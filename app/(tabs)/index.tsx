...
import { SideBySideComparison, type SelectedVideo } from "@/components/side-by-side-comparison";
...
export default function HomeScreen() {
  ...
  const pickVideo = async (slot: VideoSlot) => {
    ...
    if (!result.canceled) {
      ...
      if (slot === "side") setShowComparison(true);
    }
  };
  ...
  const runPoseDetection = async () => {
    ...
    if (result.status === "rejected") { setPoseStatus(`저장하지 않았습니다. 품질 게이트 실패: ${result.reason}`); return; }
    if (result.status === "native_build_required") { setPoseStatus(result.reason); return; }
    setPoseStatus(`Pose detection 오류: ${result.reason}`);
  };
      ...
      <ScrollView contentContainerStyle={[styles.page, narrow && styles.pageNarrow]} showsVerticalScrollIndicator={false}>
        <View style={styles.hero}>
          ...
          <View style={styles.metaRow}>
            <Pressable onPress={() => router.push("/profile" as never)} style={({ pressed }) => [[styles.pill, styles.okPill], pressed && styles.pressed]}><Text style={styles.okPillText}>{ready ? "MY PROFILE" : "CONNECTING…"}</Text></Pressable>
        ...
        <View style={styles.block}>
          ...
          <View style={[styles.uploadGrid, narrow && styles.oneColumn]}>
            {(Object.keys(SLOT_COPY) as VideoSlot[]).map((slot) => <VideoDrop key={slot} slot={slot} value={videos[slot]?.name} onPress={() => pickVideo(slot)} />)}
          </View>
          ...
          {videos.side ? <Pressable onPress={() => setShowComparison((value) => !value)} style={({ pressed }) => [styles.compareButton, pressed && styles.pressed]}><Text style={styles.compareButtonText}>{showComparison ? "나란히 비교 닫기" : "내 영상과 참조 모션 비교"}</Text></Pressable> : null}
        ...
        <View style={styles.block}>
          ...
          {videos.side && showComparison ? <SideBySideComparison video={videos.side} reference={selected.reference} hand={hand} /> : null}
...
const styles = StyleSheet.create({
  ...
  uploadGrid: { flexDirection: "row", gap: 12 }, drop: { backgroundColor: "#F8FAFC", borderColor: "#DBE3EE", borderRadius: 0, borderStyle: "dashed", borderWidth: 2, flex: 1, minHeight: 126, padding: 15 }, dropSelected: { backgroundColor: "#FFF7ED", borderColor: "#F97316", borderStyle: "solid" }, dropTitle: { color: "#0F172A", fontFamily: "BarlowCondensed-Bold", fontSize: 22, letterSpacing: 0.8 }, dropDetail: { color: "#64748B", fontFamily: "Barlow", fontSize: 12, marginTop: 1 }, dropFile: { alignItems: "center", flexDirection: "row", gap: 6, marginTop: 14 }, dropFileText: { color: "#64748B", flex: 1, fontFamily: "Barlow", fontSize: 12 }, uploadBoundary: { color: "#64748B", fontFamily: "Barlow", fontSize: 12, lineHeight: 18, marginTop: 12 }, compareButton: { alignItems: "center", alignSelf: "flex-start", borderColor: "#16A34A", borderWidth: 2, marginTop: 12, minHeight: 40, justifyContent: "center", paddingHorizontal: 13 }, compareButtonText: { color: "#15803D", fontFamily: "BarlowCondensed-Bold", fontSize: 15, letterSpacing: 0.4 }, detectButton: { alignItems: "center", alignSelf: "flex-start", backgroundColor: "#EFF6FF", borderColor: "#1E3A5F", borderWidth: 2, flexDirection: "row", gap: 7, marginTop: 10, minHeight: 40, justifyContent: "center", paddingHorizontal: 13 }, detectButtonText: { color: "#1E3A5F", fontFamily: "BarlowCondensed-Bold", fontSize: 15, letterSpacing: 0.4 }, poseNotice: { alignItems: "flex-start", backgroundColor: "#EFF6FF", borderLeftColor: "#1E3A5F", borderLeftWidth: 4, flexDirection: "row", gap: 7, marginTop: 10, padding: 10 }, poseNoticeText: { color: "#1E3A5F", flex: 1, fontFamily: "Barlow", fontSize: 12, lineHeight: 18 }, disabled: { opacity: 0.45 },
