...
export const PERSISTED_MOTION_PACKET_JOINTS_V1 = Object.freeze([...PERSISTED_JOINT_NAMES_V2]);
...
function assertDenseArray(value: unknown, count: number, label: string): asserts value is unknown[] {
  if (!Array.isArray(value) || value.length !== count) fail(`${label} count must be ${count}`);
  // Include symbols and non-enumerable properties: only indices and length are public.
  if (Reflect.ownKeys(value).length !== count + 1) fail(`${label} contains unsupported or missing fields`);
  for (let index = 0; index < count; index += 1) {
    if (!Object.prototype.hasOwnProperty.call(value, index)) fail(`${label} must not be sparse`);
  }
}

function canonicalPhase(frameIndex: number): number {
  return frameIndex / (FRAME_COUNT - 1);
}
...
function assertCanonicalPacket(value: unknown): asserts value is MotionPacketV1 {
  ...
  assertDenseArray(value.joints, JOINT_COUNT, "joints");
  if (value.joints.some((joint, index) => joint !== PERSISTED_MOTION_PACKET_JOINTS_V1[index])) fail("joint order is not canonical");
  assertDenseArray(value.phaseAnchors, ANCHOR_COUNT, "phase anchors");
  ...
  assertDenseArray(value.frames, FRAME_COUNT, "frames");
  value.frames.forEach((frame, frameIndex) => {
    ...
    if (frame.phase !== canonicalPhase(frameIndex)) {
...
export function decodeMotionPacketV1(input: Uint8Array): MotionPacketV1 {
  ...
  for (let frameIndex = 0; frameIndex < FRAME_COUNT; frameIndex += 1) {
    const phase = canonicalPhase(frameIndex);
    // Only one wire code is valid for each canonical sample. Returning the exact
    // semantic grid (also used by anchors) makes decoded packets valid encode input.
    if (view.getUint16(offset, true) !== quantizePhase(phase)) fail(`frame ${frameIndex} phase is invalid`);
    offset += 2;
...
export function buildMotionPacketV1(profileInput: RepresentativePose4DV2, shootingHand: ShootingHandV2): MotionPacketV1 {
  ...
  const packet: MotionPacketV1 = {
    ...
    joints: [...PERSISTED_MOTION_PACKET_JOINTS_V1],
    phaseAnchors: CANONICAL_MOTION_PACKET_ANCHORS_V1.map((anchor) => ({ ...anchor })),
    frames: profile.frames.map((frame, frameIndex) => ({
      // The private codec permits tiny floating-point drift on this fixed grid.
      phase: canonicalPhase(frameIndex),
      joints: Object.fromEntries(PERSISTED_MOTION_PACKET_JOINTS_V1.map((joint) => [joint, {
        x: frame.joints[joint].x,
        y: frame.joints[joint].y,
        z: frame.joints[joint].z,
      }])) as Record<MotionPacketJointV1, Vector3V1>,
    })),
  };
  assertCanonicalPacket(packet);
  return packet;
