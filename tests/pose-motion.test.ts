...
describe("approved actual optical-mocap pose motion", () => {
  it("uses a validated five-phase measured motion with a high follow-through", () => {
    ...
    expect(PLAYER_MONOCULAR_3D_ANALYSES).toHaveLength(2);
    ...
    expect(PLAYER_MONOCULAR_3D_ANALYSES[1]).toMatchObject({
      ...
      boundary: "monocular_relative_pose_not_metric_3d",
      sourcePhaseTimestampsMs: [0, 97, 355, 645, 806],
    });
    expect(PLAYER_MONOCULAR_3D_ANALYSES[1].formMatch?.find((check) => check.id === "release_wrist_height")?.status).toBe("match");
    expect(PLAYER_MONOCULAR_3D_ANALYSES.map((analysis) => analysis.motion.boundary)).toEqual(["monocular_relative_pose_not_metric_3d", "monocular_relative_pose_not_metric_3d"]);
