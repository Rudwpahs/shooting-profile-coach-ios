...
- the honest client publishes the profile head last only after every staging write is acknowledged or server-observed, while Rules require a matching completed revision head;
...
Firestore Rules can prove owner identity, exact keys, metadata, path IDs, byte type, byte length, and that publication points to a matching revision head. They cannot decode arbitrary binary slots or enumerate/prove that all 101 sequence chunks and five summaries exist. Therefore a hostile authenticated owner could write arbitrary same-size bytes, omit child documents, or publish self-declared child counts inside that owner's own private profile. The trusted app writes every staging document before the head, and the strict reader rejects malformed, missing, duplicate, or extra children. The fixed-size owner-private envelope limits cross-user disclosure and payload expansion, but it is not server attestation. App Check plus a trusted server-side validation/attestation path is required before treating owner-written bytes as authoritative comparative-player data.
