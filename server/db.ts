...
export async function getPersonalProfile(userId: number) {
  const db = await getDb();
  if (!db) return undefined;
  const result = await db.select().from(personalProfiles).where(eq(personalProfiles.userId, userId)).limit(1);
  return result[0];
}

export async function upsertPersonalProfile(userId: number, displayName: string) {
  const db = await getDb();
  if (!db) throw new Error("Personal profile storage is unavailable.");
  await db.insert(personalProfiles).values({ userId, displayName, privacy: "private" }).onDuplicateKeyUpdate({ set: { displayName, privacy: "private" } });
  return getPersonalProfile(userId);
}

export async function listPersonalPoseAnalyses(userId: number) {
  const db = await getDb();
  if (!db) return [];
...
export async function deletePersonalPoseAnalysis(userId: number, id: number) {
  const db = await getDb();
  if (!db) throw new Error("Personal pose storage is unavailable.");
  await db.delete(personalPoseAnalyses).where(and(eq(personalPoseAnalyses.id, id), eq(personalPoseAnalyses.userId, userId)));
}
